...
public class EstadisticaService {
    ...
    public List<EstadisticaPartidoDTO> obtenerEstadisticas() {
        ...
        // Mapa para organizar: partidoId -> { LOCAL -> count, EMPATE -> count, VISITANTE -> count }
        ...
        return partidos.stream().map(p -> {
            Map<Prediccion.ResultadoPrediccion, Long> matchVotes = statsMap.getOrDefault(p.getId(), Collections.emptyMap());
            ...
            return new EstadisticaPartidoDTO(
                    p.getNumero(),
                    p.getEquipoLocal() != null ? p.getEquipoLocal().getNombre() : "TBD",
                    p.getEquipoVisitante() != null ? p.getEquipoVisitante().getNombre() : "TBD",
                    local,
                    empate,
                    visitante,
                    total
