...
public interface MencionRepository extends JpaRepository<Planilla, Long> {
    ...
    @Query(value = """
            ...
            -- Subconsulta unida por JOIN para saber cuántos partidos válidos hubo por cada día calendario
            JOIN (
                SELECT
                    CAST(pa2.fecha_hora AS DATE) AS fecha_partido,
                    COUNT(pa2.id) AS total_partidos_dia
                FROM partidos pa2
                JOIN resultados r2 ON r2.partido_id = pa2.id
                GROUP BY CAST(pa2.fecha_hora AS DATE)
            ) info_dia ON info_dia.fecha_partido = CAST(pa.fecha_hora AS DATE)

            WHERE pl.confirmada = true
              AND pr.prediccion = r.resultado
            GROUP BY
                CAST(pa.fecha_hora AS DATE),
                pl.id,
                u.nombre,
                u.apellido,
                pl.codigo,
                info_dia.total_partidos_dia -- Al agregarlo acá es 100% legal usarlo abajo
            HAVING COUNT(pr.id) = info_dia.total_partidos_dia
            ORDER BY fecha ASC
            """, nativeQuery = true)
    ...
    /**
     * Contra la corriente: participantes que acertaron votando diferente a la
     * mayoría.
     * Retorna: [nombre, apellido, codigoPlanilla, vecesContraCorreinte]
     */
    @Query("""
            SELECT
                u.nombre,
            ...
            LIMIT 1
            """)
    List<Object[]> findContraLaCorriente();

    /**
     * Racha en llamas: predicciones ordenadas por número de partido para calcular
     * la racha más larga de aciertos consecutivos en Java.
     * Retorna: [codigoPlanilla, nombre, apellido, numeroPartido, esAcierto]
     */
    @Query("""
            SELECT
                pl.codigo,
                ...
                u.apellido,
                pa.numero,
                CASE WHEN pr.prediccion = r.resultado THEN true ELSE false END
            FROM Prediccion pr
            JOIN pr.planilla pl
            JOIN pl.usuario u
            JOIN pr.partido pa
            JOIN Resultado r ON r.partido = pa
            WHERE pl.confirmada = true
            ORDER BY pl.codigo ASC, pa.numero ASC
            """)
    List<Object[]> findPrediccionesOrdenadas();

    /**
     * El Tapado: top 10 por puntos, devuelve el de mayor codigo (último en
     * registrarse).
     * Retorna: [nombre, apellido, codigoPlanilla, puntos]
     */
    @Query("""
            ...
            LIMIT 10
            """)
    List<Object[]> findTop10Puntajes();

    /**
     * Especialista X2: mayor porcentaje de aciertos en partidos con multiplicador >
     * 1.
     * Solo considera planillas que hayan predicho al menos 1 partido x2 con
     * resultado.
     * Retorna: [nombre, apellido, codigoPlanilla, aciertosX2, totalX2ConResultado]
     */
    @Query("""
            SELECT
                u.nombre,
                u.apellido,
                pl.codigo,
                SUM(CASE WHEN pr.prediccion = r.resultado THEN 1 ELSE 0 END),
                COUNT(pr.id)
            FROM Prediccion pr
            JOIN pr.planilla pl
            JOIN pl.usuario u
            ...
            ORDER BY
                SUM(CASE WHEN pr.prediccion = r.resultado THEN 1 ELSE 0 END) * 1.0 / COUNT(pr.id) DESC,
                SUM(CASE WHEN pr.prediccion = r.resultado THEN 1 ELSE 0 END) DESC
            LIMIT 1
            """)
    List<Object[]> findEspecialistaX2();

    /**
     * El Arranque: planillas que acertaron los 3 primeros partidos.
     * Retorna: [nombre, apellido, codigoPlanilla]
     */
    @Query("""
            SELECT
                u.nombre,
